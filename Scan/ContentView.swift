                    ...
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 150, maximum: 220), spacing: 14)], spacing: 14) {
                        ForEach(viewModel.pages) { page in
                            PageThumbnail(page: page, selected: page.id == viewModel.selectedPageID)
                                ...
                                .contextMenu { Button("Auto-Align") { Task { viewModel.selectedPageID = page.id; await viewModel.alignSelectedPage() } }; Button("Rotate 90°") { Task { viewModel.selectedPageID = page.id; await viewModel.rotateSelectedPage() } }; Button("Delete", role: .destructive) { Task { viewModel.selectedPageID = page.id; await viewModel.deleteSelectedPage() } } }
                        }
                    }.padding(.vertical, 4)
                }
            }
