...
`ImageProcessingSettings.whitenPaper` and blank-page removal share `ScanImageProcessor.inkStatistics`: the page is downsampled to 400 pixels wide (channel mean in an RGB bitmap, so the levels match what is later multiplied), the outer 3 percent of each edge is skipped, the median is taken as the paper level and the fraction darker than paper minus 90 as ink. `isBlank` requires a paper-like median (at least 180) and less than 0.2 percent ink; the previous fixed threshold (samples below 245) never fired on real paper, which the iX1600 delivers at 230–245. `whitenPaper` multiplies the page by `253 / paperLevel` (capped at 1.25, only for paper-like pages), which is how plain paper ends up white as in ScanSnap Home; measured on the same sheet, the scanner's internal gamma curve gives paper at 241–242 against 231 with the downloaded linear table, raw and hardware-JPEG transfers being identical, so the whitening is needed either way. The three default profiles enable it; saved profiles keep it off until chosen.

`ImageProcessingSettings.deskew` (`PageAlignment`, "Deskew and crop to page" in the inspector) is the software counterpart of ScanSnap Home's automatic alignment: the ScanSnap iX models offer no hardware deskew or crop (their VPD page reports `hybrid crop/deskew` and `skew check` as unsupported, and SANE only has `hwdeskewcrop` for fi-series models with IPC), and their backing is white, so the page has to be found against a white background. `PageAlignment.align` asks Vision's `VNDetectDocumentSegmentationRequest` (then `VNDetectRectanglesRequest`) for the sheet's quadrilateral, accepts it when it covers at least 30 percent of the scan, is tilted by at most 12 degrees and has opposite edges of similar length, takes the tilt as the mean angle of its four edges, rotates the image about its centre onto a white canvas of the same size, rotates the corners with it and crops to the largest axis-aligned rectangle inside them, inset by 0.6 percent of the shorter side (about 1.3 mm on A4) so the edge shadow disappears. Without a usable outline the tilt comes from the content: the image is downsampled to 600 pixels wide, pixels darker than 160 are projected onto rows for trial angles from -5 to 5 degrees in 0.2-degree steps, and the angle with the highest projection variance wins; the crop is then the inscribed rectangle of the rotated image bounds. Measured on iX1600 scans at 300 dpi: 0.3–0.8 s per A4 page, outline found on every sample (tilts 0–1 degree). It replaces the earlier deskew (rectangle detection, rotation onto an enlarged transparent canvas, no crop); the alignment runs first in `ScanImageProcessor.process`, `autoCrop` ("Crop to content", the bounding box of non-white pixels) follows it, and the processor no longer rescales cropped or rotated output back to the frame's original width (the metadata correction now compares the decoded size with the frame size only). The same settings drive `Edit > Auto-Align Page` through `ScannerWorkspaceViewModel.alignSelectedPage()`, which re-processes the stored page at its own resolution. `PageAlignmentTests` checks synthetic tilted sheets and offers an opt-in harness (`SCAN_ALIGN_SAMPLES`/`SCAN_ALIGN_OUTPUT`) that aligns real scans and writes a report.
